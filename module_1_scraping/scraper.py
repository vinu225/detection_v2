"""
module_1/scraper.py — Fetch full article content from arbitrary URLs.
...
from shared.utils import extract_domain, validate_url
from shared.constants import SCRAPER_MIN_CONTENT_LENGTH, SCRAPER_DEFAULT_USER_AGENT
...
def scrape_article(url: str, use_playwright: bool = False) -> RawArticle:
    ...
    if len(article.content or "") >= SCRAPER_MIN_CONTENT_LENGTH:
        return article
...
def _scrape_webbaseloader(url: str) -> RawArticle:
    ...
    return RawArticle(
        title=metadata.get("title", extract_domain(url)),
...
def _scrape_playwright(url: str) -> RawArticle:
    ...
    doc = docs[0]
    return RawArticle(
        title=doc.metadata.get("title", extract_domain(url)),
